...
function Home() {
  ...
  useEffect(() => {
    axios
      .get(API_URL, { params: { search, limit: 6 } })
      .then((res) => {
  ...
  return (
    <div className="p-4 md:flex gap-6 max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              {grouped[c]?.map((item) => (
                <Link
                  key={item._id}
                  to={`/items/${item._id}`}
                  className="border rounded-xl shadow hover:shadow-lg bg-white overflow-hidden"
                >
                  {item.images[0] && (
                    <img
                      src={item.images[0]}
      ...
      <a
        ...
        className="fixed bottom-6 left-6 bg-green-500 text-white p-4 rounded-full shadow-lg flex items-center justify-center text-2xl hover:bg-green-600"
      >
        💬
      </a>
    </div>
...
function CategoryPage() {
  ...
  useEffect(() => {
    axios
      .get(API_URL, {
        params: { category, all: true, page, limit: 6 },
      })
      .then((res) => {
        setItems(res.data.items || []);
        setPages(Math.ceil((res.data.items?.length || 0) / 6)); // simple pages
      })
      .catch(console.error);
  }, [category, page]);
      ...
      {items.length === 0 ? (
        ...
        <>
          ...
          <Pagination
            ...
            onPageChange={(p) => {
              setPage(p);
              navigate(`/category/${category}?page=${p}`);
            }}
          />
        </>
      )}
    ...
    <div className="p-6 max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div>
        ...
        <p className="text-gray-500 mb-4">
          Pieces available: {item.piecesAvailable}
        </p>
...
function ItemForm({ mode = "create" }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: mode === "create" ? "Created!" : "Updated!",
      });
      setTimeout(
        () => navigate(mode === "create" ? "/" : `/items/${id}`),
        1500
      );
    } catch {
    ...
    <div className="p-6 max-w-lg mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <input
          ...
          className="w-full border p-3 rounded-md"
          value={form.piecesAvailable}
          onChange={(e) =>
            setForm({ ...form, piecesAvailable: e.target.value })
          }
        />
        <input
          type="file"
